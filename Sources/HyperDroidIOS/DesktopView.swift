            ...
            ZStack(alignment: .bottom) {
                ...
                if controller.startMenuVisible {
                    Color.black.opacity(0.001)
                        ...
                        .onTapGesture { controller.startMenuVisible = false }
                        .zIndex(9000)
                ...
                HDTaskbarView(
                    ...
                    onToggleStart: { controller.startMenuVisible.toggle() },
...
private struct HDWindowView: View {
    ...
    private var size: CGSize {
        state.maximized ? CGSize(width: desktopSize.width, height: desktopSize.height - taskbarHeight) : state.size
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Group {
                ...
                default: HDPlaceholderApp(kind: state.kind)
                }
            }
        }
        ...
        .overlay(RoundedRectangle(cornerRadius: state.maximized ? 0 : 3).stroke(palette.border, lineWidth: 1))
    ...
    private var titlebar: some View {
        ...
        .gesture(
            DragGesture(minimumDistance: state.maximized ? 10000 : 1)
                .onChanged { value in onFocus(); drag = value.translation }
                .onEnded { value in onMove(value.translation); drag = .zero }
        )
    }

    private func titleButton(_ asset: String, danger: Bool = false, action: @escaping () -> Void) -> some View {
...
private struct HDPlaceholderApp: View {
    let kind: HDWindowKind
    @Environment(\.colorScheme) private var scheme

    var body: some View {
        let p = HDPalette(scheme: scheme)
        VStack(spacing: 14) {
            HDImage(name: kind.asset).frame(width: 64, height: 64)
            Text(kind.title).font(.system(size: 18, weight: .semibold))
            Text("UI shell ported. App-specific behavior comes in the next pass.")
                .font(.system(size: 12))
