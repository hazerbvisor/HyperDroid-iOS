...
struct HDTaskbarView: View {
    ...
    var body: some View {
        ZStack {
            p.taskbar
            Rectangle().fill(p.border.opacity(0.65)).frame(height: 1).frame(maxHeight: .infinity, alignment: .top)
            ...
            HStack(spacing: 0) {
                ...
                HStack(spacing: 0) {
                    HDTaskbarIcon(asset: "app_startmenu_btn", size: metrics.taskbarAppSize, active: startMenuVisible, action: onToggleStart)
                    ForEach([HDAppEntry.builtIns[1], HDAppEntry.builtIns[0], HDAppEntry.builtIns[2]]) { app in
                        ...
                        HDTaskbarIcon(asset: app.asset,
                                      size: metrics.taskbarAppSize,
                                      active: open?.id == activeWindowID) {
                            if let open { onFocus(open.id) } else { onOpen(app.kind) }
                        }
                    }
                }
                .padding(.vertical, 3)
                ...
                HStack(spacing: 0) {
                    HDBitmapButton(asset: "ui_taskbar_btn_more", size: 18, p: p)
                        .padding(.horizontal, 8)
                    HStack(spacing: 6) {
                        HDImage(name: "ui_tb_globe_prohibited_24_regular", template: true, tint: p.text).frame(width: 18, height: 18)
                        HDImage(name: "ui_tb_speaker_2_24_regular", template: true, tint: p.text).frame(width: 18, height: 18)
                        HDImage(name: "ui_tb_battery_10_24", template: true, tint: p.text).frame(width: 18, height: 18)
                    }
                    .padding(.horizontal, 8)
    ...
    var body: some View {
        Button(action: action) {
            ZStack(alignment: .bottom) {
                ...
                if active {
                    Capsule().fill(Color(red: 0.0, green: 0.47, blue: 0.84)).frame(width: 15, height: 3).padding(.bottom, 3)
                }
            }
...
private struct HDBitmapButton: View {
    ...
    var body: some View {
        Button(action: {}) {
            HDImage(name: asset, template: true, tint: p.text).frame(width: size, height: size)
        }.buttonStyle(.plain)
    }
}
